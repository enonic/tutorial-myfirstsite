...
We now have some content, but no preview or rendering of the items.
To support server-side rendering, XP provides a server side https://developer.enonic.com/docs/xp/stable/runtime/framework[Javascript framework and controllers].

A page component is essentially is a JavaScript file that handles requests such as *GET* and *POST*.
Optionally, controllers may use templating engines to simplify output of HTML (or even other results when desired)

=== Create page component

. In your project, create a folder called `hello`: `src/main/resources/site/pages/hello/`.
. Place a controller file inside the component folder:
+
.Controller: src/main/resources/site/pages/hello/hello.js
[source,javascript]
----
include::code/pages/hello/hello.js[]
----
+
. Once the file is in place, redeploy the app:
+
  enonic project deploy
+

NOTE: Each page controller must reside in its own folder under the `site/pages` directory.
  The name of the controller JavaScript file must be the same as the directory that contains it.
  The HTML view file can reside anywhere in the project and have any valid file name.
  As such, templates may easily shared between components.

=== Setup page

Now, lets map the page component to our content.

. Click edit on the site
. From the preview panel, select the "Hello" page component

You should now see something like this in the preview area:

image::images/todo.png["Simple static web page saying 'Hello world'", width=734px]
