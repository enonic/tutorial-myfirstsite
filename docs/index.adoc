...
When the application started, it automatically created a Content Studio project called `My First Site`, and imported some sample content. Let's have a closer look:
...
. Start off by **adding a component descriptor**. The descriptor statically declears the region:
+
...
And you should see this glorious result.

image::css.png["Countries page in dark mode after adding the css", width=1024px]

TIP: There were only two changes in this file, one line that includes the css file, and a small attribute on the html element to activate dark mode support.
